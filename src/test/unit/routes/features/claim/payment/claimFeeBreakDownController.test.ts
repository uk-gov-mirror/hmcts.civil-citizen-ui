...
describe('on GET', () => {
  ...
  it('should clear paymentSyncError on the journey cache', async () => {
    ...
    claim.claimFee = {calculatedAmountInPence: 10000, code: 'FEE0202', version: 1};
  ...
  it('should return 500 status code when error occurs', async () => {
    (getClaimById as jest.Mock).mockRejectedValueOnce(new Error('Failed to get claim'));
    await request(app)
...
describe('on POST', () => {
  ...
  it('should enable the warning text if payment request is failed', async () => {
    ...
    await request(app)
      .post(CLAIM_FEE_BREAKUP).expect((res: request.Response) => {
        expect(res.status).toBe(302);
